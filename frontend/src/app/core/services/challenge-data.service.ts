...
export class ChallengeDataService {
  ...
  private toFriendlyError(error: unknown): Observable<never> {
    if (!(error instanceof HttpErrorResponse)) {
      return throwError(() => new Error('The challenge request failed. Please try again.'));
    }
        ...
        () =>
          new Error(
            this.extractServerMessage(error) ??
              'The backend rejected this challenge. Check the required fields.'
          )
      );
    }
    ...
    return throwError(
      () =>
        new Error(
          this.extractServerMessage(error) ?? `Challenge request failed with status ${error.status}.`
        )
    );
  }

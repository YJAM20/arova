...
export class FuturePlanDataService {
  ...
  private toFriendlyError(error: unknown): Observable<never> {
    if (!(error instanceof HttpErrorResponse)) {
      return throwError(() => new Error('The future plan request failed. Please try again.'));
    }
        ...
        () =>
          new Error(
            this.extractServerMessage(error) ??
              'The backend rejected this future plan. Check the required fields.'
          )
      );
    }
    ...
    return throwError(
      () =>
        new Error(
          this.extractServerMessage(error) ?? `Future plan request failed with status ${error.status}.`
        )
    );
  }

...
export class SongDataService {
  ...
  private toFriendlyError(error: unknown): Observable<never> {
    if (!(error instanceof HttpErrorResponse)) {
      return throwError(() => new Error('The song request failed. Please try again.'));
    }
    ...
    if (error.status === 400) {
      return throwError(
        () =>
          new Error(
            this.extractServerMessage(error) ?? 'The backend rejected this song. Check the fields.'
          )
      );
    }
    ...
    return throwError(
      () =>
        new Error(
          this.extractServerMessage(error) ?? `Song request failed with status ${error.status}.`
        )
    );
  }
